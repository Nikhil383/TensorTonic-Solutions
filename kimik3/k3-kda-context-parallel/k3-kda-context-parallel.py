...
def kda_context_parallel(transition_matrices: torch.Tensor, local_contributions: torch.Tensor, initial_state: torch.Tensor) -> dict[str, torch.Tensor]:
    """
    Returns a dictionary containing incoming states, outgoing states, and the final global state.
    """
    ...
    return {"incoming_states": torch.stack(incoming_states), "outgoing_states": torch.stack(outgoing_states), "final_state": outgoing_states[-1]}